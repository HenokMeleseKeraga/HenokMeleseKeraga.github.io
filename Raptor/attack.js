...
function splat(gender) {
    var peeps = document.querySelectorAll("#people ." + gender);
    for (var i = 0; i < peeps.length / 5; i++) {
        var randomIndex = Math.floor(Math.random() * peeps.length);
        peeps[randomIndex].classList.remove(gender);
        peeps[randomIndex].classList.add("splat");   // so future kills won't choose splat victims
    }
}
